"""
Nine Men's Morris - PPO Trainer for Random Opponent Training
Simplified trainer without curriculum - just trains against random for 10M epochs
"""
...
class PPOTrainer:
    """PPO trainer for random opponent training."""
    ...
    def get_learning_rate(self) -> float:
        ...
        cfg = self.config
        # Decay LR over training
        progress = min(1.0, self.episode_count / cfg.total_episodes)
        lr = cfg.lr_policy * (1.0 - 0.9 * progress)  # Decay to 10% of initial
        return max(lr, 1e-6)
    ...
    def start_workers(self):
        ...
        # Use bounded queues to prevent memory explosion
        self.request_queue = mp.Queue(maxsize=1000)
        self.experience_queue = mp.Queue(maxsize=5000)
    ...
    def collect_experiences(self, target_episodes: int) -> Tuple[List[ExperienceBatch], List[float]]:
        ...
        while len(all_experiences) < target_episodes:
            ...
            try:
                # Limit how many we grab at once to prevent memory buildup
                for _ in range(100):
                    ...
                    # Track wins
                    if batch.game_result > 0.5:
                        self.recent_wins.append(1)
                    elif batch.game_result < -0.5:
                        self.recent_wins.append(0)
                    else:
                        self.recent_wins.append(0.5)

                    if len(all_experiences) >= target_episodes:
                        break
    ...
    def update_policy(self, experiences: List[ExperienceBatch]) -> Dict:
        ...
        del all_obs, all_actions, all_old_logprobs, all_old_values, all_masks
    ...
    def log_progress(self, returns: List[float], metrics: Dict):
        ...
        win_rate = np.mean(self.recent_wins) if self.recent_wins else 0.0

        elapsed = time.time() - self.start_time
        eps_per_sec = self.episode_count / elapsed if elapsed > 0 else 0
        progress = self.episode_count / self.config.total_episodes * 100

        # Calculate ETA
        remaining_episodes = self.config.total_episodes - self.episode_count
        if eps_per_sec > 0:
            remaining_seconds = remaining_episodes / eps_per_sec
            eta = datetime.now() + timedelta(seconds=remaining_seconds)
            eta_str = eta.strftime("%Y-%m-%d %H:%M:%S")
        else:
            eta_str = "N/A"

        # Do minimax evaluation
        max_depth_beaten, minimax_str = self.evaluate_vs_minimax_progressive()

        print(
            f"[{progress:5.1f}%] "
            f"Ep {self.episode_count:,}/{self.config.total_episodes:,} | "
            f"WR:{win_rate:.0%} | "
            f"Ret: {avg_return:+.3f} | "
        ...
        if self.log_file is None:
            path = os.path.join(self.config.log_dir, f"random_train_{time.strftime('%Y%m%d_%H%M%S')}.csv")
            self.log_file = open(path, 'w', newline='')
            self.log_writer = csv.DictWriter(self.log_file, fieldnames=[
                'episode', 'progress', 'steps', 'avg_return', 'ema_return', 'win_rate',
                'policy_loss', 'value_loss', 'entropy', 'kl_div', 'clip_frac', 'grad_norm', 'lr',
                'minimax_depth_beaten'
            ])
    ...
    def save_checkpoint(self, prefix="checkpoint"):
        ...
        path = os.path.join(
            ...
            f"{prefix}_ep{self.episode_count}_{time.strftime('%Y%m%d_%H%M%S')}.pt"
        )
        torch.save({
            'episode': self.episode_count,
    ...
    def load_checkpoint(self, path: str):
        ...
        print(f"Loaded from episode {self.episode_count}")

    def train(self):
        """Main training loop - 10M epochs against random, then minimax eval."""
        cfg = self.config

        print("=" * 70)
        print("Nine Men's Morris - Random Opponent Training (10M epochs)")
        print("=" * 70)
        print(f"Device: {self.device}")
        print(f"Workers: {cfg.num_workers} x {cfg.envs_per_worker} envs = {cfg.num_workers * cfg.envs_per_worker} parallel")
        print(f"Target: {cfg.total_episodes:,} episodes")
        print("=" * 70)
        ...
        try:
            while self.episode_count < cfg.total_episodes:
                ...
                # Free experience memory immediately after update
                del experiences
                gc.collect()
            ...
            # Save final model
            final_path = os.path.join(cfg.model_dir, f"final_{time.strftime('%Y%m%d_%H%M%S')}.pt")
            ...
            elapsed = time.time() - self.start_time
            print(f"\nTraining complete: {self.episode_count:,} episodes in {elapsed / 3600:.1f}h ({self.episode_count / elapsed:.0f}/s)")

            # Final comprehensive minimax evaluation
            self.final_minimax_evaluation()

    def final_minimax_evaluation(self):
        """Comprehensive minimax evaluation at the end of training."""
        print("\n" + "=" * 70)
        print("FINAL MINIMAX EVALUATION")
        print("=" * 70)
        ...
        for depth in range(1, 7):
            ...
            games_to_play = 50  # More games for final eval
            ...
            print(f"  Depth {depth}: {wins:2d}W / {draws:2d}D / {losses:2d}L  |  Win Rate: {win_rate:5.1%}  |  {status}")
