...
def test_shared_move_cache_basic_get_put():
    cache = fastnmm.SharedMoveCache.create("/test_nmm_basic", 1024 * 1024)
    try:
        assert cache.get(0xDEADBEEFCAFEBABE) == -1
        cache.put(0xDEADBEEFCAFEBABE, 42)
        cache.put(0x1234567890ABCDEF, 7)
        assert cache.get(0xDEADBEEFCAFEBABE) == 42
        assert cache.get(0x1234567890ABCDEF) == 7
        # key=0 is the empty sentinel, never stored.
        cache.put(0, 999)
...
def test_shared_move_cache_attach_sees_creator_writes():
    name = "/test_nmm_attach"
    creator = fastnmm.SharedMoveCache.create(name, 1024 * 1024)
    try:
        creator.put(0xABCDABCDABCDABCD, 17)
        attacher = fastnmm.SharedMoveCache.attach(name)
        try:
            assert attacher.get(0xABCDABCDABCDABCD) == 17
            assert attacher.num_entries == creator.num_entries
            assert not attacher.is_creator
        finally:
            attacher.close()
    finally:
        creator.close()
...
def test_engine_writes_to_root_cache_after_search():
    name = "/test_nmm_engine_write"
    cache = fastnmm.SharedMoveCache.create(name, 1024 * 1024)
    try:
        eng = fastnmm.MinimaxEngine(16 * 1024 * 1024)
        eng.set_root_cache(cache)
        game = fastnmm.load_game("nine_mens_morris")
        state = game.new_initial_state()
        result = eng.search(state, 3)
        key = fastnmm.zobrist_key(state)
        assert cache.get(key) == result.action
...
def test_engine_uses_root_cache_for_move_ordering():
    """If the cache has a hit, the engine evaluates that move first.
    Result must match the no-cache search (alpha-beta is sound)."""
    name = "/test_nmm_hint"
    cache = fastnmm.SharedMoveCache.create(name, 1024 * 1024)
    try:
        game = fastnmm.load_game("nine_mens_morris")
        ...
        eng_no = fastnmm.MinimaxEngine(16 * 1024 * 1024, strict_parity=True)
        r_no = eng_no.search(state, 4)

        eng_yes = fastnmm.MinimaxEngine(16 * 1024 * 1024, strict_parity=True)
        eng_yes.set_root_cache(cache)
        # Pre-populate cache with the "right" move.
        cache.put(fastnmm.zobrist_key(state), r_no.action)
        r_yes = eng_yes.search(state, 4)
        assert r_no.action == r_yes.action
...
def test_engine_set_root_cache_none_detaches():
    name = "/test_nmm_detach"
    cache = fastnmm.SharedMoveCache.create(name, 1024 * 1024)
    try:
        eng = fastnmm.MinimaxEngine(16 * 1024 * 1024)
        eng.set_root_cache(cache)
        eng.set_root_cache(None)
        # After detach, search must still work and not touch the cache.
        game = fastnmm.load_game("nine_mens_morris")
        state = game.new_initial_state()
        eng.search(state, 3)
