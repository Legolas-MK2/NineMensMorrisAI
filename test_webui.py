...
# This matches the pyspiel observation tensor layout
POINT_TO_COORD = {
    0: (0, 0), 1: (0, 3), 2: (0, 6),
    3: (1, 1), 4: (1, 3), 5: (1, 5),
    6: (2, 2), 7: (2, 3), 8: (2, 4),
...
# Adjacent positions for each point
ADJACENCY = {
    0: [1, 7], 1: [0, 2, 9], 2: [1, 3],
...
def get_ai_move_with_probs(model, state, player: int, temperature: float = 0.4) -> Tuple[int, Dict[int, float]]:
    ...
    with torch.no_grad():
        ...
        for act in legal_actions:
            move_probs[act] = float(display_probs[0, act].item())
...
def get_minimax_move(state, depth: int) -> int:
    """Get minimax move with specified depth."""
    bot = MinimaxBot(max_depth=depth)
    return bot.get_action(state)
...
def api_move():
    ...
    # Get current player and phase before move
    player = game_state.state.current_player()
    state_str_before = str(game_state.state)
    is_capture = "Capture time" in state_str_before
